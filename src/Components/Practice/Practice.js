...
const Practice = () => {
  ...
  useEffect(() => {
    fetch(
      `https://code-type-webapp-default-rtdb.firebaseio.com/para/${num}.json`
    )
      .then((res) => {
        return res.json();
      })
    ...
    if (new_str.length === givenText.length) {
      if (new_str === givenText) {
        setNotificationMessage(
          "Congratulations, You have successfully typed the given text !!"
        );
        setNotificationType("success");
        setIsNotificationHidden(false);
      } else {
        setNotificationMessage(
          "You FAILED ( typed incorrect text )... Try Again"
        );
  ...
  return (
    <div className="practice" onClick={fun}>
      ...
      />
      <button className="backToHomeScreen" onClick={goToHomeScreen}>
        Home
      </button>
      ...
      <div className="second">
        {aLoading ? (
          ...
          <>
            <p className="message">{isCapsOn ? "CapsLock is ON" : message}</p>
            <button id="text-box">
              {code
                ? code.map((ch, i) => (
                    <span data-id={i} key={i}>
                      {ch}
                    </span>
                  ))
                : "Loading Text..."}
        ...
        <div className="buttons">
          <button className="restart" onClick={highlighFirstChar}>
            Restart
          </button>
          <button className="generate" onClick={handleRefresh}>
